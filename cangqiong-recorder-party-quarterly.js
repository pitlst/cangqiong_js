 ...
 * 右下角出现「季度党群绩效行数」后再执行 __cqPqRec.copy()，不要一贴完就复制。
 * 列表 formId 会从官方 loadData 自动发现，不必事先写死。
 *
 * 命令：
 *   window.__cqPqRec.help()
 *   window.__cqPqRec.copy()                   复制记录（请求 + 点击 + 解析到的 pageId）
 *   window.__cqPqRec.download()               下载 cq-pq-rec.json
 *   window.__cqPqRec.fetchPartyQuarterly()    点应用/党费/季度党群绩效贡献度并解析列表
...
(function () {
  ...
  function clipResponse(q, text) {
    var max = MAX_BODY;
    if (q && q.ac === "loadData" && q.f && !isReservedForm(q.f)) max = 800000;
  ...
  function isListLoadData(r) {
    ...
    if (r.pageId && String(r.pageId).indexOf(TARGET.menuItemId) === 0) return true;
  ...
  function fetchPartyQuarterly() {
    ...
    setStatus("开始拉季度党群绩效：点应用 → 点党费 → 打开季度党群绩效贡献度 → 读 loadData");
    fetching = Promise.resolve()
      ...
      .then(function (res) {
        ...
        var parsed = extractTable(payload);
        if (!parsed.rows.length) {
          for (var i = rec.requests.length - 1; i >= 0; i--) {
            var r = rec.requests[i];
            if (!isListLoadData(r)) continue;
            rememberListReq(r);
            parsed = extractTable(r.response);
            payload = r.response;
            if (parsed.rows.length) break;
          }
        }
        rec.partyColumns = parsed.columns;
        rec.partyRows = parsed.rows;
        rec.partyRawPreview = clip(typeof payload === "string" ? payload : JSON.stringify(payload), 2000);
        setStatus(
          "季度党群绩效行数 " +
            parsed.rows.length +
            (TARGET.dataFormId ? "，formId=" + TARGET.dataFormId : "") +
            (parsed.rows.length ? "，可 __cqPqRec.copy()" : "，未解析到行")
        );
        if (parsed.columns.length) console.log(TAG, "列", parsed.columns);
        if (parsed.rows.length) console.table(parsed.rows.slice(0, 20));
        else console.log(TAG + " 未解析到 billlistap 行，预览", rec.partyRawPreview);
        return parsed.rows;
      })
      .then(
        function (rows) {
  ...
  function snapshot() {
    ...
    return {
      ...
      note: fetching
        ? "仍在拉数，请等控制台出现「季度党群绩效行数」或右下角状态变化后再 copy"
  ...
  function help() {
    console.log(
      [
        TAG + " 命令",
        "  __cqPqRec.copy()                    复制请求/操作/季度党群绩效解析结果",
        "  __cqPqRec.download()                下载 cq-pq-rec.json",
        "  __cqPqRec.fetchPartyQuarterly()     点「应用/党费/季度党群绩效贡献度」走官方会话，再解析 loadData",
        "  __cqPqRec.autoFetch = false         禁止登录后自动拉数",
        "  __cqPqRec.stop()                    停止记录",
        "粘贴后请保持 Context 在顶层主控台。右下角出现「季度党群绩效行数」后再 copy。",
      ].join("\n")
    );
  }
  ...
  setStatus("已挂钩。主控台 pageId 就绪后会自动拉季度党群绩效，请勿立刻 copy");
  help();
