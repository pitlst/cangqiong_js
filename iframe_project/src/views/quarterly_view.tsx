...
import { calculate_quarterly_cxzy_eval, calculate_quarterly_party_eval, calculate_quarterly_party_score, DuplicateOrgConfigError, SubmittedPeriodBillError, quarter_key, type EvalBill } from '@/lib/calc_quarterly_eval'
...
function make_quarterly_columns(onCommitAdmin: (bill: ParseBillRow, next: string) => Promise<void>) {
...
export function QuarterlyView() {
    ...
    const [calcKind, setCalcKind] = useState<'score' | 'eval' | 'cxzy'>('eval')
    ...
    const quarterlyColumns = useMemo(() => make_quarterly_columns(saveAdminEval), [saveAdminEval])
    ...
    async function confirmCalcScore() {
        ...
        setCalculating(true)
        const toastId = toast.loading(`正在计算${year}${quarter}党群绩效得分`)
        try {
            const { bills, delete_billnos } = await calculate_quarterly_party_score(year, quarter)
            if (!bills.length) {
    ...
    async function confirmCalcEval() {
        ...
        try {
            ...
            if (!bills.length) {
                if (tied_reason) {
                    toast.warning('无法计算', { id: toastId, description: tied_reason })
                    return
                }
                toast.warning('没有可计算的党组织', { id: toastId })
                return
